---
search:
  exclude: true
---

<!-- atlas-route -->
> **Current study page: [Genetics + evolution + systems biology](../../learn/ts-f19.md)**
>
> The earlier reference below is retained for continuity.
<!-- /atlas-route -->

# TS-F19  -  Genetics + evolution + systems biology

**Branch:** Technical Systems
**Status:** live
**Prerequisites:** TS-F07

## Exit capability

Reason about inheritance, variation, population change, gene circuits, network motifs, stochastic expression and evolutionary robustness.

## Core concepts



## Deletion test



## Mastery gate

Reconstruct -> transfer -> break -> bound -> check -> defend

## Formation modes

R, S, P, M, B, V, L

## AI modes

A0, A1, A2, A4, A5

## Entry sources

- **primary-spine**  -  Genetics: A Conceptual Approach
